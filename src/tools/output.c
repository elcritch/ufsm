...
static void ufsm_gen_states(struct ufsm_state* state)
{
    ...
    {
        fprintf(fp_c,
                "static struct ufsm_entry_exit %s = {\n",
                id_to_decl(e->id));
        if (flag_strip)
        {
            fprintf(fp_c, "  .id     = \"\", \n");
...
static void ufsm_gen_regions(struct ufsm_region* region)
{
    for (struct ufsm_region* r = region; r; r = r->next)
    {
        ...
        {
            fprintf(fp_c,
                    "static struct ufsm_transition %s = {\n",
                    id_to_decl(t->id));
            if (flag_strip)
            {
                fprintf(fp_c, "  .id     = \"\", \n");
                     ...
                     bool strip)
{
    ...
    {
        fprintf(fp_c,
                "struct ufsm_machine * get_%s(void) { return &%s; }\n",
                m->name,
                id_to_decl(m->id));
    }
